...
- setInputValues(data: Record<string, string>):void - принимает объект с данными для полей формы
- setError(data: {field: string, value: string, validInformation: string}):void - принимает объект с данными для отображения текстов ошибок полей ввода
...
- setItems(items: TBasketItem[]): void - отображает элементы в списке корзины
- addItem(item: TBasketItem): void - добавляет один элемент в список корзины
- setTotal(total: number): void - устанавливает суммарную сумму товаров
- removeItem(id: string): void - удаляет из списка товар по его id
...
#### Класс CardsContainerView
Класс отвечает за отображение блока с карточками на главной странице. 
constructor(containerSelector: string) - конструктор принимает селектор контейнера, в который вставляются карточки
...
- addCard(cardElement: HTMLElement): void - добавляет карточку в конец контейнера
